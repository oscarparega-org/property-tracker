...
] as const satisfies ReadonlyArray<{ status: PropertyDto["decisionStatus"]; label: string; description: string; tone: string }>;

const statusLabels = Object.fromEntries(lifecycle.map(item => [item.status, item.label])) as Record<PropertyDto["decisionStatus"], string>;
  ...
  return (
    <section className="board-shell" aria-label="Proceso de propiedades">
      <div className="board-intro">
        ...
        <div className="board-legend"><span /><strong>{properties.length}</strong> activas en el proceso</div>
      </div>
        ...
        {lifecycle.map((stage, index) => {
          ...
          return (
            <section
              ...
              onDragOver={event => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; setOverStatus(stage.status); }}
            ...
            >
              <header className="kanban-column-heading">
                ...
                <div><h2 id={`stage-${stage.status}`}>{stage.label}</h2><p>{stage.description}</p></div>
              ...
              <div className="kanban-cards">
                {cards.map(property => (
                  <article
                    ...
                    onDragEnd={() => { setDraggedId(null); setOverStatus(null); }}
                  >
                    <div className="kanban-card-image">
                      {property.images[0] ? <img src={property.images[0].url} alt="" /> : <div className="image-placeholder">Sin foto</div>}
                      {property.isFavorite ? <span className="kanban-favorite"><MaterialIcon name="favorite" /></span> : null}
                    ...
                    <div className="kanban-card-actions">
                      <label>
                        ...
                        >
                          {lifecycle.map(option => <option key={option.status} value={option.status}>{statusLabels[option.status]}</option>)}
                        </select>
                      </label>
                      <Link href={`/properties/${property.id}`} aria-label={`Ver ${property.title}`}><MaterialIcon name="arrowForward" /></Link>
                    </div>
                  </article>
                ))}
