    ...
    <header className="border-b border-slate-200 bg-white/85 backdrop-blur">
      <nav className="mx-auto flex h-16 max-w-5xl items-center justify-between px-6" aria-label="Main navigation">
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          {!isPending && !session ? (
            <>
              <Link className="font-medium text-slate-700 hover:text-slate-950" href="/sign-in">
                Sign in
              </Link>
              <Link
                className="rounded-lg bg-slate-950 px-3 py-2 font-semibold text-white hover:bg-slate-700"
                href="/sign-up"
              >
                Sign up
              </Link>
          ...
          {!isPending && session ? (
            <>
              ...
              <Link className="font-medium text-slate-700 hover:text-slate-950" href="/dashboard">
                Dashboard
              </Link>
              <button
                className="rounded-lg border border-slate-300 px-3 py-2 font-semibold hover:bg-slate-100"
                onClick={signOut}
                type="button"
              >
                Sign out
