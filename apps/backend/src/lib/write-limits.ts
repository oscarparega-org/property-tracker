...
export async function reserveWrite(db: PrismaClient | Prisma.TransactionClient, ownerId: string, headers: Headers, action: string, perHour: number, globalPerDay: number): Promise<void> {
  if ('$transaction' in db) return db.$transaction(tx => reserveWrite(tx, ownerId, headers, action, perHour, globalPerDay));
  ...
  const ip = process.env.TRUST_PROXY === 'true' ? headers.get('x-forwarded-for')?.split(',')[0]?.trim() || 'unknown' : 'unknown';
  const ipHash = createHmac('sha256', secret || 'local-development').update(ip).digest('hex');
  const tx = db;
    ...
    const [account, address, global] = await Promise.all([
      ...
      ip === 'unknown' ? 0 : tx.mutationAudit.count({ where: { ipHash, action, createdAt: { gte: new Date(now - 3_600_000) } } }),
      tx.mutationAudit.count({ where: { action, createdAt: { gte: new Date(now - 86_400_000) } } }),
    ]);
    if (account >= perHour || address >= perHour || global >= globalPerDay) throw new HTTPException(429, { message: 'Se alcanzó el límite temporal. Intenta más tarde.' });
