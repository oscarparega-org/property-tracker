...
export function buildEnhancementPreview(current: PropertyDto, candidate: PropertyInput, importId: string): EnhancementPreviewDto {
  const changes: PreviewChange[] = [];
  const add = (field: string, label: string, currentValue: unknown, proposed: unknown, replaceOther = false) => {
    const currentMissing = currentValue === null || currentValue === undefined || currentValue === "" || (replaceOther && currentValue === "OTHER");
    const proposedValue = printable(proposed);
    if (currentMissing && proposedValue !== null && proposedValue !== "" && proposedValue !== "OTHER") changes.push({ field, label, current: printable(currentValue), proposed: proposedValue });
  };
  ...
  add("maintenanceCurrency", "Moneda de mantenimiento", current.maintenanceCurrency, candidate.property.details.maintenanceCurrency);
  ...
  if (phoneAdds.length) changes.push({ field: "agentPhones", label: "Teléfonos del asesor", current: current.agentPhones.join(", ") || null, proposed: [...current.agentPhones, ...phoneAdds].join(", ") });
  const existingImages = new Set(current.images.map(image => image.url));
  const existingFeatures = new Set(current.features.map(feature => `${feature.category}:${feature.name.toLocaleLowerCase()}`));
  return {
    importId,
    propertyId: current.id,
    changes,
    addedImages: candidate.images.map(image => image.url).filter(url => !existingImages.has(url)),
    addedFeatures: candidate.features.filter(feature => !existingFeatures.has(`${feature.category}:${feature.name.toLocaleLowerCase()}`)),
  };
}
...
export async function applyEnhancement(db: PrismaClient, id: string, ownerId: string, candidate: PropertyInput) {
  await db.$transaction(async tx => {
    ...
    const scalar: Prisma.PropertyUncheckedUpdateInput = {
      ...
      sourceMetadata: { ...jsonRecord(record.sourceMetadata), latestEnhancement: candidate.source.rawMetadata as Prisma.JsonValue },
    };
    for (const field of allowed) (scalar as Record<string, unknown>)[field] = (source as Record<string, unknown>)[field];
    ...
    const existingFeatures = new Set(record.features.map(feature => `${feature.category}:${feature.name.toLocaleLowerCase()}`));
    const features = candidate.features.filter(feature => !existingFeatures.has(`${feature.category}:${feature.name.toLocaleLowerCase()}`));
    await tx.property.update({ where: { id, ownerId }, data: {
      ...scalar,
      ...(images.length ? { images: { create: images.map((image, index) => ({ url: image.url, alt: image.alt, sortOrder: record.images.length + index })) } } : {}),
      ...(features.length ? { features: { create: features.map(feature => ({ category: feature.category, name: feature.name })) } } : {}),
    } });
  });
