...
export async function saveProperty(db: PrismaClient, ownerId: string, formData: FormData, id?: string) {
  ...
  if (id) {
    return toPropertyDto(await db.property.update({
      where: { id, ownerId },
      data: { ...propertyData, images: { deleteMany: {}, create: imageRows }, features: { deleteMany: {}, create: featureRows } },
      include,
    }));
  }
  return toPropertyDto(await db.property.create({
    data: { ...propertyData, ownerId, sourceObservedAt: new Date(), images: { create: imageRows }, features: { create: featureRows } } as Prisma.PropertyUncheckedCreateInput,
    include,
  }));
}
