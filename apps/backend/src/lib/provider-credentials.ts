...
export class ProviderCredentialError extends Error {
  constructor(message: string, public readonly kind: 'AUTH' | 'UNAVAILABLE' | 'INVALID_CONFIGURATION') {
    super(message);
  }
}
...
function encryptionKey() {
  ...
  if (key.length < 32) throw new Error('PROVIDER_CREDENTIAL_ENCRYPTION_KEY must contain at least 32 bytes encoded as base64');
...
export function decryptProviderCredential(ownerId: string, provider: IntegrationProvider, setting: Pick<ProviderSetting, 'credentialCiphertext' | 'credentialIv' | 'credentialAuthTag'>) {
...
export function allowedOpenAiModels() {
  const configured = process.env.OPENAI_ALLOWED_MODELS?.split(',').map(value => value.trim()).filter(Boolean);
...
export async function validateProviderCredential(provider: IntegrationProvider, credential: string, model: string | null, fetcher: typeof fetch = fetch) {
  const url = provider === 'OPENAI'
    ? `https://api.openai.com/v1/models/${encodeURIComponent(model || '')}`
    : 'https://api.firecrawl.dev/v2/team/credit-usage';
  let response: Response;
  try {
    response = await fetcher(url, { headers: { authorization: `Bearer ${credential}`, accept: 'application/json' }, signal: AbortSignal.timeout(10_000) });
  } catch {
    throw new ProviderCredentialError('No fue posible contactar al proveedor.', 'UNAVAILABLE');
  }
  if (response.status === 401 || response.status === 403 || response.status === 404) {
    throw new ProviderCredentialError(provider === 'OPENAI' ? 'La clave no tiene acceso al modelo seleccionado.' : 'La clave de Firecrawl no es válida.', 'AUTH');
  }
...
export async function updateProviderSettings(db: PrismaClient, ownerId: string, provider: IntegrationProvider, input: ProviderSettingsUpdate) {
  const existing = await db.providerSetting.findUnique({ where: { ownerId_provider: { ownerId, provider: provider as PrismaProvider } } });
  const model = provider === 'OPENAI' ? input.model ?? existing?.model ?? allowedOpenAiModels()[0]! : null;
  if (provider === 'OPENAI' && !allowedOpenAiModels().includes(model!)) throw new ProviderCredentialError('El modelo no está permitido.', 'INVALID_CONFIGURATION');
  if (input.monthlyOperationLimit > providerGlobalLimit(provider)) throw new ProviderCredentialError('El límite supera el máximo permitido por el servidor.', 'INVALID_CONFIGURATION');
  const credential = input.credential ?? (existing ? decryptProviderCredential(ownerId, provider, existing) : null);
  if (input.enabled && !credential) throw new ProviderCredentialError('Agrega una credencial antes de activar el proveedor.', 'INVALID_CONFIGURATION');
  const modelChanged = provider === 'OPENAI' && model !== existing?.model;
  const mustValidate = Boolean(input.credential || (input.enabled && (!existing?.validatedAt || existing.needsAttention || !existing.enabled || modelChanged)));
  ...
  await db.providerSetting.upsert({
    ...
    create: {
      ownerId, provider: provider as PrismaProvider, enabled: input.enabled, model,
      monthlyOperationLimit: input.monthlyOperationLimit,
      ...(encrypted ? { credentialCiphertext: encrypted.ciphertext, credentialIv: encrypted.iv, credentialAuthTag: encrypted.authTag, credentialHint: encrypted.hint } : {}),
      validatedAt: mustValidate ? new Date() : null,
    },
    update: {
      enabled: input.enabled, model, monthlyOperationLimit: input.monthlyOperationLimit,
      ...(encrypted ? { credentialCiphertext: encrypted.ciphertext, credentialIv: encrypted.iv, credentialAuthTag: encrypted.authTag, credentialHint: encrypted.hint } : {}),
...
export async function testStoredProviderCredential(db: PrismaClient, ownerId: string, provider: IntegrationProvider) {
  const setting = await db.providerSetting.findUnique({ where: { ownerId_provider: { ownerId, provider: provider as PrismaProvider } } });
  const credential = setting ? decryptProviderCredential(ownerId, provider, setting) : null;
  if (!setting || !credential) throw new ProviderCredentialError('No hay una credencial configurada.', 'INVALID_CONFIGURATION');
  await validateProviderCredential(provider, credential, setting.model);
  await db.providerSetting.update({ where: { id: setting.id }, data: { validatedAt: new Date(), needsAttention: false } });
  return (await listProviderSettings(db, ownerId)).find(item => item.provider === provider)!;
}

export async function deleteProviderCredential(db: PrismaClient, ownerId: string, provider: IntegrationProvider) {
  await db.providerSetting.updateMany({ where: { ownerId, provider: provider as PrismaProvider }, data: {
    enabled: false, credentialCiphertext: null, credentialIv: null, credentialAuthTag: null,
    credentialHint: null, validatedAt: null, needsAttention: false,
  } });
  return (await listProviderSettings(db, ownerId)).find(item => item.provider === provider)!;
}
...
export async function enabledProviderCredential(db: PrismaClient, ownerId: string, provider: IntegrationProvider) {
  ...
  return credential ? { credential, model: setting.model, monthlyOperationLimit: setting.monthlyOperationLimit, settingId: setting.id } : null;
}

export async function markProviderCredentialInvalid(db: PrismaClient, settingId: string) {
  await db.providerSetting.updateMany({ where: { id: settingId }, data: { enabled: false, validatedAt: null, needsAttention: true } });
}
