...
function publicAddress(value: unknown) {
  ...
  return match ? { street: match[1]!.trim() || null, exteriorNumber: match[2]! } : { street: full, exteriorNumber: null };
}
...
function extract(artifact: ExtractionArtifact): DirectExtraction | null {
  ...
  const images = [...new Map(publicPictures.flatMap((item) => {
  ...
  const agentAvatarUrl = safeImage(agent?.profilePicture, /^\/property\/contact\/[a-f0-9]{24}\/profile_picture\/[A-Za-z0-9._-]+$/i);
  const phones = [...new Set([string(agent?.phone), string(company?.phone)].filter((phone): phone is string => Boolean(phone)))];
  ...
  const input = propertyInputSchema.parse({
    schemaVersion: 1,
    source: {
      provider: PROVIDER_NAME, url: artifact.url, listingId, listingKey, observedAt: new Date().toISOString(),
      rawMetadata: {
        strategy: artifact.strategy, providerKey: PROVIDER_KEY, providerVersion: PROVIDER_VERSION,
        source: "next-data", galleryExpected: publicPictures.length, galleryRetrieved: images.length,
        addressRounded: listing?.addressIsRounded === true,
      },
    },
    property: {
      title, description: string(listing?.description), propertyType: propertyType(payload.type), operationType: "SALE",
      price: { amount: number(price?.price) ?? number(listing?.value), currency },
      address: {
        street: publicStreet.street, exteriorNumber: publicStreet.exteriorNumber, interiorNumber: null,
        neighborhood, municipality, state: stateName, postalCode, countryCode: string(country?.id)?.toUpperCase() ?? "MX",
        formatted: [streetLine, neighborhood, municipality, stateName, postalCode].filter(Boolean).join(", ") || null,
      },
      coordinates,
      details: {
        landAreaM2: number(attributes?.totalSurface), constructionAreaM2: number(attributes?.roofedSurface),
        bedrooms: number(attributes?.suites), bathrooms: number(attributes?.bathrooms), parkingSpaces: number(attributes?.parkings),
        parkingType: null, serviceRoom: null,
        propertyAgeYears: yearBuild !== null && yearBuild > 1800 ? Math.max(0, new Date().getFullYear() - yearBuild) : null,
        condition: string(attributes?.condition), orientation: null, landUse: null, buildingLevels: null, unitFloor: null,
        maintenanceAmount, maintenanceCurrency: maintenanceAmount === null ? null : currency,
      },
    ...
    contact: {
      ...
      agentAvatarUrl, phones, email: string(agent?.email), officeName: string(company?.name), sourceOfficeId: string(company?._id),
    },
  });
  ...
  return {
    ...
    evidence: {
      strategy: artifact.strategy, providerKey: PROVIDER_KEY, providerVersion: PROVIDER_VERSION,
      confidenceScore: 100, gate: "PASS", hasPropertyStructuredData: true, hasPropertyLanguage: true,
      imageCandidates: images.length, galleryExpected, hasCompleteGallery,
    },
    complete: true, gate: "PASS", confidenceScore: 100,
  };
}
...
export const pulppoProvider: ImportProvider = {
  ...
  matches(url) {
    return (url.hostname === "mi.pulppo.com" || url.hostname === "pulppo.com" || url.hostname === "www.pulppo.com")
