...
function parseAssignedJson(html: string, id: string) {
  ...
  for (let index = start; index < script.length; index += 1) {
    ...
    if (character === '"') inString = true;
      ...
      try { return JSON.parse(script.slice(start, index + 1)) as JsonRecord; }
      catch { return null; }
    }
  }
  return null;
...
function mercadoLibreId(url: URL) {
  return url.pathname.match(/\b(ML[A-Z]-?\d+)\b/i)?.[1]?.replace("-", "").toUpperCase() ?? null;
}
...
function galleryImages(html: string): MercadoLibreImage[] {
  ...
  const pictures = array(record(gallery?.data)?.tabs).map(record).find((tab) => tab?.id === "pictures");
...
function splitAddress(location: JsonRecord | null) {
  ...
  const locality = string(map?.item_location)?.split(",").map((part) => part.trim()) ?? [];
  ...
  return {
    ...
    neighborhood: formattedParts.length >= 4
      ? formattedParts[1]!
...
function buildInput(artifact: ExtractionArtifact): DirectExtraction | null {
  ...
  const images = array(artifact.metadata.mercadoLibreImages).map(record).flatMap((image) => {
  ...
  const sourceOfficeId = typeof sourceOfficeIdValue === "number" || typeof sourceOfficeIdValue === "string" ? String(sourceOfficeIdValue) : null;
  ...
  const input = propertyInputSchema.parse({
    ...
    property: {
      ...
      operationType: "SALE",
      price: { amount: number(priceComponent?.value), currency: string(priceComponent?.currency_id)?.toUpperCase() ?? null },
      address: {
      ...
      details: {
        ...
        maintenanceCurrency: maintenance === null ? null : (string(specs.get("mantenimiento"))?.match(/\b[A-Z]{3}\b/)?.[0] ?? string(priceComponent?.currency_id)?.toUpperCase() ?? null),
      },
...
async function enrich(artifact: ExtractionArtifact, context: ProviderContext) {
  ...
  const counter = array(gallery?.media_counters).map(record).find((item) => item?.type === "photos");
  ...
  if (galleryUrlValue) {
    ...
    } catch { /* The listing data remains usable when the optional gallery request fails. */ }
  }
  if (!images.length) {
...
export const mercadoLibreProvider: ImportProvider = {
  ...
  matches(url) { return isMercadoLibreHost(url.hostname) && Boolean(mercadoLibreId(url)); },
  enrich,
  extract: buildInput,
};
