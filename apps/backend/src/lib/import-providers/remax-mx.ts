...
function buildApiInput(artifact: ExtractionArtifact, payloadValue: unknown): DirectExtraction | null {
  ...
  if (
    !payload ||
    !listingId ||
    string(payload.property.propiedad_id) !== listingId ||
    string(payload.property.operacion) !== '1'
  )
    return null;
  ...
  const images = [
    ...new Set(
      array(property.imagenes)
        .map((item) => safeCdnUrl(record(item)?.path, imagePattern))
        .filter((url): url is string => Boolean(url))
    )
  ];
  const featureValues = (value: unknown) =>
    array(value)
  ...
  const phones = [
    ...new Set(
      [string(office.oficina_callpicker), string(office.oficina_telefono), string(agent.celular)].filter(
...
function propertyImages(html: string, listingId: string) {
  ...
  for (const tag of html.match(/<img\b[^>]*>/gi) ?? []) {
    ...
    if (!new RegExp(`^/properties/${escapeRegex(listingId)}/[a-f0-9]+\\.(?:jpe?g|png|webp)$`, 'i').test(url.pathname))
      continue;
...
function titleFor(html: string) {
  return (
    meta(html, 'og:title') ??
    `${textByClass(html, 'jsTipo') ?? 'Propiedad'} en ${textByClass(html, 'jsOperacion') ?? 'venta'}`
  );
}
  ...
  const input = propertyInputSchema.parse({
    ...
    property: {
      ...
      address: {
        ...
        countryCode: 'MX',
        formatted:
          [street, exteriorNumber, neighborhood, municipality, state, postalCode].filter(Boolean).join(', ') || null
      },
...
export const remaxMexicoProvider: ImportProvider = {
  ...
  matches(url) {
    return (
      (url.hostname === 'remax.com.mx' || url.hostname.endsWith('.remax.com.mx')) &&
