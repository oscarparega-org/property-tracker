...
for (const [network, prefix] of [['::', 128], ['::1', 128], ['fc00::', 7], ['fe80::', 10], ['ff00::', 8], ['2001:db8::', 32], ['64:ff9b::', 96], ['2002::', 16]] as const) blocked.addSubnet(network, prefix, 'ipv6');
...
async function publicAddresses(hostname: string) {
  ...
  if (!addresses.length || addresses.some(({ address }) => isPrivateAddress(address))) throw new Error('La URL apunta a una red privada o reservada.');
  return addresses;
}
...
export const publicDispatcher = new Agent({ connect: {
