...
export function propertyRoutes(db: PrismaClient) {
  ...
  async function enhancementCapability(ownerId: string) {
    ...
    if (!firecrawl || !openai?.model)
      return {
        available: false,
        reason: 'Activa credenciales válidas de Firecrawl y OpenAI para usar la mejora profunda.'
      };
  ...
  routes.post('/properties/:id/enhancements', async (c) => {
    ...
    try {
      await assertSafePublicUrl(property.sourceUrl);
    } catch {
      throw new HTTPException(400, { message: 'La URL de origen ya no es una página pública válida.' });
    }
    const result = await db.$transaction(async (tx) => {
      ...
      const job = await tx.propertyImport.create({
        data: {
          ...
          canonicalUrl: canonicalizeListingUrl(property.sourceUrl!),
          kind: 'ENHANCEMENT'
        }
      });
      importDebug(job.id, 'enhancement.queued', {
        propertyId: property.id,
  ...
  routes.get('/enhancements/:id', async (c) => {
    const job = await db.propertyImport.findFirst({
      where: { id: c.req.param('id'), ownerId: owner(c), kind: 'ENHANCEMENT' }
    });
  ...
  routes.post('/enhancements/:id/apply', async (c) => {
    ...
    importDebug(job.id, 'enhancement.applied', {
      propertyId: job.propertyId,
      scalarChanges: preview.changes.length,
      addedImages: preview.addedImages.length,
      addedFeatures: preview.addedFeatures.length
    });
