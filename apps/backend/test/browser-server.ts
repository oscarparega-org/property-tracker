...
if (!process.env.DATABASE_URL || !new URL(process.env.DATABASE_URL).pathname.endsWith('_test')) throw new Error('Browser tests require a disposable _test database.');
...
globalThis.fetch = async (input, init) => {
  ...
  if (url === 'https://api.firecrawl.dev/v2/scrape') return Response.json({ success: true, data: {
    rawHtml: '<meta property="og:title" content="Casa de prueba"><meta property="og:image" content="https://example.com/enhanced-house.jpg">',
    markdown: 'Casa de prueba en venta, 3 recámaras, 2 baños y 150 m² de construcción.', metadata: {},
  } });
  if (url === 'https://api.openai.com/v1/responses') {
    ...
    void _rawMetadata;
    return Response.json({ output_text: JSON.stringify({ isPropertyListing: true, confidence: 0.96, evidence: ['Casa en venta con recámaras, baños y superficie'], property: { ...extracted, source } }) });
  }
