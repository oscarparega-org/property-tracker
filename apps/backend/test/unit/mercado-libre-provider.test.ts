...
function listingHtml(options: { streetLine?: string | null; formattedAddress?: string; itemLocation?: string } = {}) {
  ...
  const formattedAddress =
    options.formattedAddress ?? 'Gabriel Mancera 600, Del Valle Norte, Benito Juárez, Distrito Federal';
  ...
  return nordicHtml({
    ...
    components: {
      ...
      gallery: {
        ...
        total_count: 16,
        primary: {
          id: '700004-MLM113088407904_072026',
          src: 'https://http2.mlstatic.com/D_NQ_NP_700004-MLM113088407904_072026-F-test.webp'
        },
        secondary: [
          {
            id: '700002-MLM113088407902_072026',
            src: 'https://http2.mlstatic.com/D_NQ_NP_700002-MLM113088407902_072026-F-test.webp'
          }
        ],
        media_counters: [
          { type: 'photos', url: 'https://departamento.mercadolibre.com.mx/vis-modals/gallery/MLM5668988320' }
        ]
      },
      location: {
        ...
        map_info: {
          item_address: streetLine,
          item_location: itemLocation,
          location: { latitude: '19.389217', longitude: '-99.163804' }
        }
      },
      specifications: {
              ...
              multimedia: [
                { title: 'Sala y comedor', elements: elements.slice(0, 8) },
                {
                  ...
                  elements: [
                    ...elements.slice(8),
                    elements[0],
                    { id: 'icon', src_high: 'https://http2.mlstatic.com/icon.svg' }
                  ]
                }
              ]
            }
...
test('routes MercadoLibre listings to its code provider and other sites to generic', () => {
  assert.equal(
    resolveImportProvider(new URL('https://departamento.mercadolibre.com.mx/MLM-5668988320-test-_JM')).key,
    'mercado-libre'
  );
...
test('extracts MercadoLibre structured fields and exactly the listing gallery', async () => {
  ...
  const enriched = await mercadoLibreProvider.enrich!(artifact, {
    fetchHtml: async () => ({
      url: 'https://departamento.mercadolibre.com.mx/vis-modals/gallery/MLM5668988320',
      html: galleryHtml()
    })
  });
  ...
  assert.deepEqual(
    result.input.features.map((feature) => feature.name),
    ['Ambientes: 4', 'Bodegas: 0', 'Disposición: Contrafrente', 'Admite mascotas: No']
  );
...
test('maps a three-part MercadoLibre location to neighborhood when the street is hidden', async () => {
  const artifact: ExtractionArtifact = {
    ...
    html: listingHtml({
      streetLine: null,
      formattedAddress: 'Roma Sur, Cuauhtémoc, Distrito Federal',
      itemLocation: 'Cuauhtémoc, Distrito Federal'
    }),
    text: 'Departamento en venta',
    metadata: {}
  };
  const enriched = await mercadoLibreProvider.enrich!(artifact, {
    fetchHtml: async () => ({
      url: 'https://departamento.mercadolibre.com.mx/vis-modals/gallery/MLM5257028714',
      html: galleryHtml()
    })
  });
  const result = extractDeterministic(enriched);
