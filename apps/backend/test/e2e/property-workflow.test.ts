...
if (!url || !new URL(url).pathname.endsWith('_test')) throw new Error('TEST_DATABASE_URL must point to a disposable database ending in _test.');
...
const emails: string[] = [];
let cookieA = '', cookieB = '', ownerA = '', ownerB = '', propertyId = '';
const listingUrl = `https://example.com/listing/${randomUUID()}`;
const artifact = { url: listingUrl, provider: 'example.com', strategy: 'direct' as const, html: '<html><head><meta property="og:title" content="Casa de prueba"><script type="application/ld+json">{"@type":"House","name":"Casa de prueba","description":"Una propiedad para verificar el flujo completo.","offers":{"price":2500000,"priceCurrency":"MXN"},"geo":{"latitude":19.4,"longitude":-99.1},"image":["https://example.com/photo.jpg"]}</script></head></html>', text: 'Casa de prueba', metadata: {} };
const extracted = extractDeterministic(artifact);
const extraction = async () => ({ input: extracted.input, evidence: extracted.evidence, strategy: 'direct', provider: 'example.com', firecrawlCredits: 0 });

function request(path: string, cookie = cookieA, method = 'GET', body?: BodyInit) {
  return app.request(path, { method, headers: { Cookie: cookie, Origin: 'http://localhost:5173', ...(typeof body === 'string' ? { 'Content-Type': 'application/json' } : {}) }, body });
}
function editor(property: PropertyDto, publicationStatus = 'PUBLISHED') {
  ...
  for (const [field, category] of [['areaFeatures', 'AREA'], ['equipmentFeatures', 'EQUIPMENT'], ['otherFeatures', 'OTHER']]) form.set(field!, property.features.filter(item => item.category === category).map(item => item.name).join('\n'));
...
beforeAll(async () => {
  for (const label of ['a', 'b']) {
    ...
    emails.push(email);
    const response = await request('/api/auth/sign-up/email', '', 'POST', JSON.stringify({ name: 'Prueba', email, password: 'test-password-123' }));
    expect(response.status, await response.clone().text()).toBe(200);
    const result = await response.json();
    const cookie = response.headers.getSetCookie().map(value => value.split(';')[0]).join('; ');
    if (label === 'a') { cookieA = cookie; ownerA = result.user.id; }
    else { cookieB = cookie; ownerB = result.user.id; }
  }
});
afterAll(async () => {
...
describe('clean account → URL → private draft → publication', () => {
  ...
  it('enqueues a URL once per owner and processes it into a draft', async () => {
    const responses = await Promise.all([request('/api/imports', cookieA, 'POST', JSON.stringify({ url: listingUrl })), request('/api/imports', cookieA, 'POST', JSON.stringify({ url: listingUrl }))]);
    ...
    expect(progress.status).toBe('READY'); propertyId = progress.propertyId;
    expect(await (await request('/api/properties')).json()).toEqual([]);
  ...
  it('sets favorites, archive state, decision notes, rating and visit date', async () => {
    expect((await request(`/api/properties/${propertyId}/favorite`, cookieA, 'PATCH', JSON.stringify({ isFavorite: true }))).status).toBe(200);
    const archive = await (await request(`/api/properties/${propertyId}/archive`, cookieA, 'PATCH', JSON.stringify({ archived: true }))).json();
    expect(archive.archivedAt).toBeTruthy();
    const form = new FormData();
    for (const [key, value] of Object.entries({ decisionStatus: 'VISIT_SCHEDULED', rating: '4', notes: 'Visitar', visitAt: '2026-10-01T18:00:00.000Z', rejectionReason: '', isFavorite: 'on' })) form.set(key, value);
  ...
  it('creates and edits a manual property without a source URL', async () => {
    ...
    form.set('id', ''); form.set('sourceUrl', ''); form.set('sourceListingId', ''); form.set('sourceProvider', 'MANUAL'); form.set('title', 'Captura manual');
    ...
    const edit = editor(manual); edit.set('title', 'Captura revisada');
  ...
  it('does not overwrite a published property when another job resolves to its listing', async () => {
    const job = await db.propertyImport.create({ data: { ownerId: ownerA, url: listingUrl, canonicalUrl: listingUrl } });
  ...
  it('rejects validation errors, duplicate manual URLs, private targets and foreign origins', async () => {
    const property = await (await request(`/api/properties/${propertyId}`)).json();
    const invalid = editor(property); invalid.set('rating', '9');
    expect((await request(`/api/properties/${propertyId}`, cookieA, 'PUT', invalid)).status).toBe(400);
    const duplicate = editor(property); duplicate.set('id', '');
    expect((await request('/api/properties', cookieA, 'POST', duplicate)).status).toBe(409);
    expect((await request('/api/imports', cookieA, 'POST', JSON.stringify({ url: 'http://127.0.0.1/private' }))).status).toBe(400);
    const foreign = await app.request(`/api/properties/${propertyId}/favorite`, { method: 'PATCH', headers: { Cookie: cookieA, Origin: 'https://untrusted.invalid', 'Content-Type': 'application/json' }, body: '{"isFavorite":false}' });
  ...
  it('retries failed jobs, recovers interrupted jobs, and respects disabled provider budgets', async () => {
    ...
    expect(await db.propertyImport.findUnique({ where: { id: job.id } })).toMatchObject({ status: 'FAILED', retryCount: 3 });
    await db.propertyImport.update({ where: { id: job.id }, data: { status: 'FETCHING', retryCount: 0, processingStartedAt: new Date(0) } });
    await recoverStaleImports(db);
    expect(await db.propertyImport.findUnique({ where: { id: job.id } })).toMatchObject({ status: 'QUEUED', retryCount: 1 });
  ...
  it('stores validated provider credentials encrypted and isolates settings by owner', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => Response.json({ object: 'model' })));
    try {
      ...
      const saved = await request('/api/settings/providers/openai', cookieA, 'PUT', JSON.stringify({
        enabled: true, credential: 'sk-owner-a-secret', model: 'gpt-5.6-luna', monthlyOperationLimit: 10,
      }));
      expect(saved.status, await saved.clone().text()).toBe(200);
      expect(await saved.json()).toMatchObject({ provider: 'OPENAI', enabled: true, credentialConfigured: true, credentialHint: '••••cret', status: 'READY' });
      const raw = await db.providerSetting.findUniqueOrThrow({ where: { ownerId_provider: { ownerId: ownerA, provider: 'OPENAI' } } });
      ...
      expect(other.find((item: { provider: string }) => item.provider === 'OPENAI')).toMatchObject({ credentialConfigured: false, credentialHint: null });

      vi.stubGlobal('fetch', vi.fn(async () => new Response('', { status: 401 })));
      const rejected = await request('/api/settings/providers/openai', cookieA, 'PUT', JSON.stringify({
        enabled: true, credential: 'sk-invalid-secret', model: 'gpt-5.6-luna', monthlyOperationLimit: 10,
      }));
      expect(rejected.status).toBe(400);
      expect((await db.providerSetting.findUniqueOrThrow({ where: { ownerId_provider: { ownerId: ownerA, provider: 'OPENAI' } } })).credentialHint).toBe('cret');
  ...
  it('passes only the job owner credential to the worker and stops using it after removal', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => Response.json({ object: 'model' })));
    try {
      const saveB = await request('/api/settings/providers/openai', cookieB, 'PUT', JSON.stringify({
        enabled: true, credential: 'sk-owner-b-secret', model: 'gpt-5.6-luna', monthlyOperationLimit: 10,
      }));
    ...
    const jobA = await db.propertyImport.create({ data: { ownerId: ownerA, url: `${listingUrl}?owner=a`, canonicalUrl: `${listingUrl}?owner=a` } });
    const jobB = await db.propertyImport.create({ data: { ownerId: ownerB, url: `${listingUrl}?owner=b`, canonicalUrl: `${listingUrl}?owner=b` } });
    ...
    const withoutCredential = await db.propertyImport.create({ data: { ownerId: ownerA, url: `${listingUrl}?owner=a2`, canonicalUrl: `${listingUrl}?owner=a2` } });
  ...
  it('previews and safely applies a Firecrawl + OpenAI enhancement', async () => {
    ...
    try {
      const saved = await request('/api/settings/providers/firecrawl', cookieB, 'PUT', JSON.stringify({
        enabled: true, credential: 'fc-owner-b-secret', model: null, monthlyOperationLimit: 10,
      }));
      expect(saved.status, await saved.clone().text()).toBe(200);
      const capability = await (await request(`/api/properties/${(await db.property.findFirstOrThrow({ where: { ownerId: ownerB } })).id}/enhancement-capability`, cookieB)).json();
    ...
    const deep = vi.fn(async (...args: [string, ExtractionOptions]) => {
      void args;
      return { input: candidate, evidence: { gate: 'PASS' }, strategy: 'firecrawl+openai', provider: 'example.com', firecrawlCredits: 1 };
    });
    expect(await processImportJob(db, start.importId, deep)).toBe(true);
    expect(deep.mock.calls[0]?.[1]).toMatchObject({ mode: 'DEEP', firecrawl: { credential: 'fc-owner-b-secret' }, openai: { credential: 'sk-owner-b-secret' } });
    ...
    const updated = await db.property.findUniqueOrThrow({ where: { id: target.id }, include: { images: true, features: true } });
  ...
  it('fails a non-listing validation once and creates no property', async () => {
    ...
    const reject = async () => { throw new ListingValidationError(); };
    expect(await processImportJob(db, job.id, reject)).toBe(false);
    expect(await db.propertyImport.findUnique({ where: { id: job.id } })).toMatchObject({ status: 'FAILED', retryCount: 1, propertyId: null });
  });
});
